...
import { listReviewQueue, recordReview } from "@/lib/queries";
...
export default async function ReviewPage({
  ...
  searchParams: Promise<{ document?: string; offset?: string }>;
}) {
  ...
  return (
    <div style={{ padding: "var(--s-6)", maxWidth: 1400, margin: "0 auto" }}>
      <header style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: "var(--s-6)" }}>
        ...
        <span className="font-mono text-muted">{total} pending</span>
      </header>
      ...
      {items.length === 0 ? (
        <p className="font-body text-muted">Nothing pending. Every extracted record has been reviewed.</p>
      ) : (
      ...
      <nav style={{ display: "flex", gap: "var(--s-4)", marginTop: "var(--s-6)" }}>
        {offset > 0 && (
          <Link className="font-mono" href={`/review?offset=${Math.max(0, offset - 25)}`}>&larr; previous</Link>
        )}
        {offset + items.length < total && (
          <Link className="font-mono" href={`/review?offset=${offset + 25}`}>next &rarr;</Link>
        )}
      </nav>
    </div>
  );
